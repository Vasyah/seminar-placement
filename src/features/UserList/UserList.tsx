...
export const UserList: React.FC<PropsWithChildren<userListProps>> = ({ users = [], onUserAdd, onUserDelete }: PropsWithChildren<userListProps>) => {
    ...
    const getTabContent = () =>
        ['Все участники', 'Заселённые', 'Незаселённые'].map((tabName, idx) => ({
            label: tabName,
            key: String(idx),
            children: <UserTable users={getTabUsers(tabName, users)} onUserAdd={onUserAdd} onUserDelete={onUserDelete} />,
        }));
